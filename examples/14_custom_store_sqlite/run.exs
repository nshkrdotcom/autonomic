...
tasks =
  for n <- 1..2,
      do: Task.async(fn -> ExampleSQLiteStore.advance_epoch(concurrent_id, %{writer: n}) end)

epochs =
  tasks
  |> Enum.map(&Task.await(&1, 2_000))
  |> Enum.map(fn {:ok, epoch} -> epoch end)
  |> Enum.sort()
...
IO.puts(
  "ASSERTION PASSED: SQLite-backed example preserves monotonic epoch advance, lease revocation, and effect staling under a serialized writer"
)

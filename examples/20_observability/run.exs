...
:telemetry.attach_many(
  handler,
  [
    [:autonomic, :example, :effect, :prepare],
    [:autonomic, :example, :effect, :evaluate],
    [:autonomic, :example, :effect, :commit]
  ],
...
measure = fn stage, fun ->
  ...
  :telemetry.execute([:autonomic, :example, :effect, stage], %{duration: duration}, %{
    episode_id: spec.id
  })
...
events =
  for _ <- 1..3 do
    receive do
      message = {:telemetry_event, _, _, _} -> message
    after
      1_000 -> raise "missing telemetry event"
    end
  end
...
Support.assert!(
  Enum.all?(events, fn {:telemetry_event, _, m, md} ->
    is_integer(m.duration) and md.episode_id == spec.id
  end),
  "events contain bounded identifiers and duration only"
)

IO.puts(
  "ASSERTION PASSED: telemetry vocabulary can wrap prepare/evaluate/commit without logging payloads or credentials"
)

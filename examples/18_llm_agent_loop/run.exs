  ...
  def next(%{step: 0}),
    do:
      {:ok,
       %{
         tool: :local_exec,
         argv: ["/bin/sh", "-c", "printf agent-local > agent.txt && cat agent.txt"]
       }}

  def next(%{step: 1}),
    do: {:ok, %{tool: :effect, kind: :http_read, target_id: "demo", payload: "brokered request"}}
...
state =
  Enum.reduce_while(1..4, state, fn _, st ->
    {:ok, turn} = ExampleAgentModel.Scripted.next(st)

    case turn do
      %{tool: :local_exec, argv: argv} ->
        {:ok, execution} =
          runtime.domain.backend.exec(runtime.domain, %ExecutionDomain.Command{argv: argv})
      ...
      %{tool: :effect, kind: kind, target_id: target_id, payload: payload} ->
        effect =
          Support.prepare!(spec, runtime, kind, payload, target_id: target_id)
          |> Support.evaluate!()
          |> Support.commit!()

        IO.inspect(%{id: effect.id, state: effect.state, kind: effect.kind},
          label: "brokered tool result"
        )
...
Support.assert!(
  Enum.any?(state.transcript, &(&1.tool == :effect)),
  "loop must route an external action through EffectBroker"
)

IO.puts(
  "ASSERTION PASSED: local computation stays in ExecutionDomain; external authority crosses EffectBroker"
)

...
effect = %ProposedEffect{
  ...
  epoch: 7,
  lease_id: lease_id,
  class: :class_4_irreversible_high_impact,
  kind: :publish,
  target: %{"id" => "demo"},
  payload_ref: "sha256:payload",
  payload_digest: Canonical.hash("release"),
  revision: 3,
  reversible?: false,
  state: :evaluating,
...
Support.assert_equal!(
  Policy.verify(signed_policy, rotated_keys, "policy"),
  {:error, :invalid_signature},
  "old key absent after rotation"
)
...
IO.puts(
  "approval binds effect=#{effect.id} revision=#{effect.revision} epoch=#{effect.epoch} payload=#{effect.payload_digest}"
)

IO.puts(
  "ASSERTION PASSED: policy signatures and Class 4 approvals use separate explicit keyrings/domains and exact canonical documents"
)

...
rows = [
  ...
  {"human allow + semantic deny",
   [
     %{"source" => "human", "decision" => "allow"},
     %{"source" => "semantic", "decision" => "deny"}
   ], {:deny, "semantic"}},
  {"policy deny + semantic allow",
   [
     %{"source" => "signed_policy", "decision" => "deny"},
     %{"source" => "semantic", "decision" => "allow"}
   ], {:deny, "signed_policy"}},
  {"capability deny + human allow",
   [
     %{"source" => "capability_violation", "decision" => "deny"},
     %{"source" => "human", "decision" => "allow"}
   ], {:deny, "capability_violation"}},
  {"kernel deny + everything allow",
   [
     %{"source" => "kernel_denial", "decision" => "deny"},
     %{"source" => "signed_policy", "decision" => "allow"},
     %{"source" => "human", "decision" => "allow"},
     %{"source" => "semantic", "decision" => "allow"}
   ], {:deny, "kernel_denial"}}
]
...
expansion =
  Autonomic.AuthorityGovernor.issue(spec.id, %{
    authority_source: :semantic,
    capabilities: [],
...
Support.assert_equal!(
  expansion,
  {:error, :semantic_cannot_expand_authority},
  "semantic authority expansion"
)

signal =
  Enum.reduce(1..3, nil, fn seq, _last ->
...
Support.assert!(
  match?({:homeostat, action, _, _} when action in [:narrow, :preempt, :yield], signal),
  "sustained semantic drift must only contract or pause authority"
)

IO.puts(
  "ASSERTION PASSED: higher-precedence denial wins and semantics cannot expand hard authority"
)

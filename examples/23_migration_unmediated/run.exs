...
File.rm_rf!(naive_workspace); File.mkdir_p!(naive_workspace)
File.write!(Path.join(naive_workspace, "result.txt"), "direct shell write")
:ok = TargetStore.put({"external", "release"}, "DIRECT-MUTATION")
Support.assert_equal!(TargetStore.get({"external", "release"}), "DIRECT-MUTATION", "naive direct external mutation occurred without policy")
...
TargetStore.reset()
Support.install_adapter!("http_read", :class_2_external_observable_or_compensatable, %{"id" => "demo", "object_key" => "release"})
...
{:ok, execution} = runtime.domain.backend.exec(runtime.domain, %ExecutionDomain.Command{argv: ["/bin/sh", "-c", "printf mediated > mediated.txt && cat mediated.txt"]})
...
Support.assert_equal!(committed.state, :committed, "brokered mutation commits after policy/evaluation")
Support.assert_equal!(TargetStore.get({"demo", "release"}), committed.payload_digest, "target receives only committed digest")

IO.puts("STEP 1 local shell -> ExecutionDomain")
IO.puts("STEP 2 direct external client -> EffectBroker proposal/evaluate/commit")
IO.puts("ASSERTION PASSED: migration removes the direct authoritative mutation path instead of leaving a bypass beside the broker")

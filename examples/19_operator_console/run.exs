...
IO.puts(
  "episode=#{episode.id} state=#{episode.state} epoch=#{episode.current_epoch} regime=#{homeostat.regime}"
)

Enum.each(effects, fn e ->
  IO.puts("effect=#{e.id} class=#{e.class} state=#{e.state} kind=#{e.kind}")
end)
...
Support.assert!(
  match?({:contained, _}, EpisodeController.state(spec.id)),
  "operator containment must reach terminal contained state"
)

IO.puts(
  "ASSERTION PASSED: operator read paths expose authoritative state and containment is an explicit control action"
)

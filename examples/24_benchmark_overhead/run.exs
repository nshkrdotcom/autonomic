...
samples =
  for n <- 1..10 do
    {prepare_ms, effect} =
      measure.(fn -> Support.prepare!(spec, runtime, :http_read, "bench-#{n}") end)
...
report = %{
  ...
  prepare_ms_avg: avg.(:prepare_ms),
  evaluate_ms_avg: avg.(:evaluate_ms),
  commit_ms_avg: avg.(:commit_ms),
  environment: "UnsafeLocalDomain + MemoryStore + MemoryAdapter; NOT production"
}
...
Support.assert!(
  Enum.all?(
    Map.take(report, [:prepare_ms_avg, :evaluate_ms_avg, :commit_ms_avg]) |> Map.values(),
    &(&1 >= 0.0)
  ),
  "timings must be measured"
)

IO.puts(
  "ASSERTION PASSED: kernel bookkeeping is measured separately and explicitly labeled so local-dev numbers cannot be mistaken for Linux/Postgres/model latency"
)

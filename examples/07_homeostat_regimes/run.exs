...
{:ok, _} =
  MemoryStore.create_episode(%{
    ...
    policy_version: 1,
    hard_envelope: hard,
    origin_intent_digest: Canonical.digest("homeostat"),
...
obs = fn seq, sensor, value ->
  ...
  IO.puts(
    "#{seq}: regime=#{state.regime} drift=#{Float.round(state.drift, 3)} uncertainty=#{Float.round(state.uncertainty, 3)} volatility=#{Float.round(state.volatility, 3)} -> #{inspect(signal)}"
  )
...
{:ok, _} =
  MemoryStore.create_episode(%{
    ...
    policy_version: 1,
    hard_envelope: hard,
    origin_intent_digest: Canonical.digest("hysteresis"),
    trajectory_version: 0,
    trajectory_regime: :stable
  })

defmodule Autonomic.Dev.MemoryStore do
  ...
  def set_episode_state(id, state, metadata \\ %{}), do: GenServer.call(__MODULE__, {:set_episode_state, id, state, metadata})
  def set_episode_domain(id, domain), do: GenServer.call(__MODULE__, {:set_episode_domain, id, domain})

  @impl true
  def advance_epoch(episode_id, metadata), do: GenServer.call(__MODULE__, {:advance_epoch, episode_id, metadata})
  ...
  @impl true
  def put_effect(%ProposedEffect{} = effect), do: GenServer.call(__MODULE__, {:put_effect, effect})
  ...
  def list_effects(episode_id, states \\ nil), do: GenServer.call(__MODULE__, {:list_effects, episode_id, states})
  ...
  def update_effect_revision(id, attrs), do: GenServer.call(__MODULE__, {:update_effect_revision, id, attrs})
  ...
  def record_effect_decision(id, decision), do: GenServer.call(__MODULE__, {:record_effect_decision, id, decision})
  def effect_decisions(id, revision \\ nil), do: GenServer.call(__MODULE__, {:effect_decisions, id, revision})
  def begin_effect_commit(id, required), do: GenServer.call(__MODULE__, {:begin_effect_commit, id, required})
  def mark_committing(id), do: transition(id, :committing, :effect_committing)
  def complete_effect(id, outcome, details \\ %{}), do: GenServer.call(__MODULE__, {:complete_effect, id, outcome, details})
  def reconcile_effect(id, outcome, details \\ %{}), do: GenServer.call(__MODULE__, {:reconcile_effect, id, outcome, details})
  def pending_reconciliation, do: GenServer.call(__MODULE__, :pending_reconciliation)

  def put_checkpoint(%EpisodeCheckpoint{} = checkpoint), do: GenServer.call(__MODULE__, {:put_checkpoint, checkpoint})
  def latest_stable_checkpoint(episode_id), do: GenServer.call(__MODULE__, {:latest_stable_checkpoint, episode_id})
  def checkpoint(id), do: GenServer.call(__MODULE__, {:checkpoint, id})
  def update_trajectory(episode_id, state), do: GenServer.call(__MODULE__, {:update_trajectory, episode_id, state})
  ...
  def claim_episode(id, node, ttl_ms), do: GenServer.call(__MODULE__, {:claim_episode, id, node, ttl_ms})
  def renew_episode_owner(id, token, ttl_ms), do: GenServer.call(__MODULE__, {:renew_episode_owner, id, token, ttl_ms})
  ...
  def handle_call({:current_epoch, id}, _from, state) do
    reply = case state.episodes[id] do nil -> {:error, :episode_not_found}; episode -> {:ok, episode.current_epoch} end
    {:reply, reply, state}
  end
  ...
  def handle_call({:create_episode, attrs}, _from, state) do
    ...
    if Map.has_key?(state.episodes, id) do
      ...
      {seq, state} = add_event(state, id, :episode_created, %{epoch: episode.current_epoch, policy_version: episode.policy_version})
      _ = seq
  ...
  def handle_call({:fetch_episode, id}, _from, state) do
    reply = case state.episodes[id] do nil -> :not_found; episode -> {:ok, episode} end
    {:reply, reply, state}
  end
  ...
  def handle_call({:set_episode_state, id, new_state, metadata}, _from, state) do
    with {:ok, episode} <- fetch_map(state.episodes, id, :episode_not_found) do
      ...
      state = put_in(state, [:episodes, id], episode)
      {_seq, state} = add_event(state, id, :episode_state, %{state: new_state, metadata: plain(metadata)})
      {:reply, :ok, state}
    else
  ...
  def handle_call({:set_episode_domain, id, nil}, _from, state) do
    with {:ok, episode} <- fetch_map(state.episodes, id, :episode_not_found) do
      episode = %{episode | domain_ref: nil, domain_generation: nil, domain_os_ref: nil, domain_metadata: %{}}
      state = put_in(state, [:episodes, id], episode)
  ...
  def handle_call({:set_episode_domain, id, domain}, _from, state) do
    with {:ok, episode} <- fetch_map(state.episodes, id, :episode_not_found),
      ...
      episode = %{episode | domain_ref: domain.id, domain_generation: domain.generation, domain_os_ref: domain.os_ref, domain_metadata: domain.metadata || %{}}
      state = put_in(state, [:episodes, id], episode)
      {_seq, state} = add_event(state, id, :domain_bound, %{domain_ref: domain.id, epoch: domain.epoch, generation: domain.generation})
      {:reply, :ok, state}
    else
  ...
  def handle_call({:advance_epoch, id, metadata}, _from, state) do
    with {:ok, episode} <- fetch_map(state.episodes, id, :episode_not_found) do
      ...
      in_flight = effects |> Map.values() |> Enum.filter(&(&1.episode_id == id and &1.epoch < new_epoch and &1.state in @reconcilable)) |> Enum.map(& &1.id)
      {_seq, state} = add_event(state, id, :epoch_advanced, %{old_epoch: new_epoch - 1, new_epoch: new_epoch, in_flight_commits: in_flight, metadata: plain(metadata)})
  ...
  def handle_call({:put_lease, lease}, _from, state) do
    with {:ok, episode} <- fetch_map(state.episodes, lease.episode_id, :episode_not_found),
      ...
      {_seq, state} = add_event(state, lease.episode_id, :lease_issued, %{lease_id: lease.id, epoch: lease.epoch, max_effect_class: lease.max_effect_class})
      {:reply, :ok, state}
    else
  ...
  def handle_call({:fetch_lease, id}, _from, state) do
    reply = case state.leases[id] do nil -> :not_found; lease -> {:ok, lease} end
    {:reply, reply, state}
  end
  ...
  def handle_call({:revoke_lease, id, reason}, _from, state) do
    case state.leases[id] do
      ...
      lease ->
        ...
        {_seq, state} = add_event(state, lease.episode_id, :lease_revoked, %{lease_id: id, reason: to_string(reason)})
        {:reply, :ok, state}
    end
  end
  ...
  def handle_call({:put_effect, effect}, _from, state) do
    with {:ok, episode} <- fetch_map(state.episodes, effect.episode_id, :episode_not_found),
      ...
      {_seq, state} = add_event(state, effect.episode_id, :effect_proposed, %{effect_id: effect.id, revision: effect.revision, class: effect.class, kind: effect.kind, payload_digest: effect.payload_digest})
      {:reply, :ok, state}
    else
  ...
  def handle_call({:prepare_existing_effect, id}, _from, state) do
    with {:ok, effect} <- fetch_map(state.effects, id, :effect_not_found),
         ...
         :ok <- check(lease.episode_id == effect.episode_id and lease.epoch == effect.epoch, :stale_lease),
         :ok <- check(is_nil(lease.revoked_at) and lease.expires_at > Canonical.now(), :expired_or_revoked_lease),
         :ok <- check(effect.version_vector.epoch == episode.current_epoch, :stale_version_epoch),
         :ok <- check(effect.version_vector.policy_version == episode.policy_version, :stale_policy),
         :ok <- check(effect.version_vector.trajectory_version == episode.trajectory_version, :stale_trajectory),
         :ok <- check(effect.version_vector.effect_revision == effect.revision, :stale_effect_revision) do
      effect = %{effect | state: :prepared}
      state = put_in(state, [:effects, id], effect)
      {_seq, state} = add_event(state, effect.episode_id, :effect_prepared, %{effect_id: id, revision: effect.revision})
  ...
  def handle_call({:list_effects, episode_id, states}, _from, state) do
    values = state.effects |> Map.values() |> Enum.filter(&(&1.episode_id == episode_id)) |> Enum.sort_by(& &1.created_at)
  ...
  def handle_call({:abort_effect, id, reason}, _from, state) do
    with {:ok, effect} <- fetch_map(state.effects, id, :effect_not_found) do
      cond do
        effect.state in [:aborted, :expired, :stale, :failed] -> {:reply, {:ok, effect}, state}
        effect.state in @reconcilable -> {:reply, {:error, :commit_horizon_crossed}, state}
        EffectState.allowed?(effect.state, :aborted) ->
          effect = %{effect | state: :aborted, failure: %{reason: to_string(reason)}}
          state = put_in(state, [:effects, id], effect)
          {_seq, state} = add_event(state, effect.episode_id, :effect_aborted, %{effect_id: id, reason: to_string(reason)})
          {:reply, {:ok, effect}, state}
        true -> {:reply, {:error, {:invalid_effect_transition, effect.state, :aborted}}, state}
      end
    else
      error -> {:reply, error, state}
    end
  ...
  def handle_call({:fetch_effect, id}, _from, state) do
    reply = case state.effects[id] do nil -> :not_found; effect -> {:ok, effect} end
    {:reply, reply, state}
  end
  ...
  def handle_call({:update_effect_revision, id, attrs}, _from, state) do
    with {:ok, effect} <- fetch_map(state.effects, id, :effect_not_found),
      ...
      effect = %{effect | revision: revision, target: Map.get(attrs, :target, effect.target), payload_ref: Map.get(attrs, :payload_ref, effect.payload_ref), payload_digest: Map.get(attrs, :payload_digest, effect.payload_digest), version_vector: vector, state: :prepared}
      state = put_in(state, [:effects, id], effect)
      {_seq, state} = add_event(state, effect.episode_id, :effect_revised, %{effect_id: id, revision: revision, payload_digest: effect.payload_digest})
      {:reply, {:ok, effect}, state}
    else
      {:error, reason} -> {:reply, {:error, reason}, state}
    end
  ...
  def handle_call({:transition, id, to, kind}, _from, state) do
    with {:ok, effect} <- fetch_map(state.effects, id, :effect_not_found),
      ...
      state = put_in(state, [:effects, id], effect)
      {_seq, state} = add_event(state, effect.episode_id, kind, %{effect_id: id, from: from, to: to})
      {:reply, {:ok, effect}, state}
    else
  ...
  def handle_call({:record_effect_decision, id, decision}, _from, state) do
    with {:ok, effect} <- fetch_map(state.effects, id, :effect_not_found),
      ...
      existing = Map.get(state.decisions, id, []) |> Enum.find(&(&1.effect_revision == effect.revision and &1.kind == entry.kind and &1.source_ref == entry.source_ref))

      cond do
        is_nil(existing) ->
          decisions = Map.update(state.decisions, id, [entry], &(&1 ++ [entry]))
          state = %{state | decisions: decisions}
          {_seq, state} = add_event(state, effect.episode_id, :effect_decision, %{effect_id: id, revision: effect.revision, kind: entry.kind, decision: entry.decision, evidence_ref: entry.evidence_ref})
  ...
  def handle_call({:effect_decisions, id, revision}, _from, state) do
    ...
    values = if is_nil(revision), do: values, else: Enum.filter(values, &(&1.effect_revision == revision))
    {:reply, {:ok, values}, state}
  end
  ...
  def handle_call({:begin_effect_commit, id, required}, _from, state) do
    with {:ok, effect} <- fetch_map(state.effects, id, :effect_not_found),
      ...
      effect = %{effect | state: :commit_intent, commit_attempt_id: attempt, idempotency_key: effect.idempotency_key || "autonomic-#{effect.id}"}
      state = put_in(state, [:effects, id], effect)
      {_seq, state} = add_event(state, effect.episode_id, :effect_commit_intent, %{effect_id: id, revision: effect.revision, commit_attempt_id: attempt, idempotency_key: effect.idempotency_key, version_vector: Map.from_struct(effect.version_vector)})
      {:reply, {:ok, effect}, state}
    else
      {:error, reason} -> {:reply, {:error, reason}, state}
    end
  ...
  def handle_call({:complete_effect, id, outcome, details}, _from, state) do
    with {:ok, effect} <- fetch_map(state.effects, id, :effect_not_found) do
      {to, attrs, event} = case outcome do
        :committed -> {:committed, %{committed_at: Canonical.now(), external_receipt_ref: Map.get(details, :receipt_ref), failure: nil, metadata: put_receipt(effect.metadata, Map.get(details, :receipt, %{}))}, :effect_committed}
        :failed -> {:failed, %{failure: Map.get(details, :failure, details)}, :effect_failed}
        :commit_unknown -> {:commit_unknown, %{failure: Map.get(details, :failure, details)}, :effect_commit_unknown}
      end
      ...
      if EffectState.allowed?(effect.state, to) do
        ...
        state = put_in(state, [:effects, id], effect)
        {_seq, state} = add_event(state, effect.episode_id, event, %{effect_id: id, from: from, to: to, details: plain(details)})
        {:reply, {:ok, effect}, state}
      else
  ...
  def handle_call({:reconcile_effect, id, outcome, details}, _from, state) do
    with {:ok, effect} <- fetch_map(state.effects, id, :effect_not_found),
      ...
      effect = case outcome do
        :committed -> %{effect | state: :committed, committed_at: Canonical.now(), failure: nil, metadata: put_receipt(effect.metadata, Map.get(details, :receipt, %{}))}
        :not_committed -> %{effect | state: :ready, commit_attempt_id: nil, failure: nil}
        :unknown -> %{effect | state: :commit_unknown, failure: Map.get(details, :failure, %{reason: :reconciliation_unknown})}
        :failed -> %{effect | state: :failed, failure: Map.get(details, :failure, details)}
      end
      state = put_in(state, [:effects, id], effect)
      {_seq, state} = add_event(state, effect.episode_id, :effect_reconciled, %{effect_id: id, outcome: outcome, details: plain(details)})
      {:reply, {:ok, effect}, state}
    else
  ...
  def handle_call(:pending_reconciliation, _from, state) do
    effects = state.effects |> Map.values() |> Enum.filter(&(&1.state in @reconcilable)) |> Enum.sort_by(& &1.created_at)
  ...
  def handle_call({:put_checkpoint, checkpoint}, _from, state) do
    with {:ok, episode} <- fetch_map(state.episodes, checkpoint.episode_id, :episode_not_found),
      ...
      episode = if checkpoint.trust_level == :stable, do: %{episode | current_checkpoint_id: checkpoint.id}, else: episode
      state = %{state | checkpoints: checkpoints} |> put_in([:episodes, episode.id], episode)
      {_seq, state} = add_event(state, episode.id, :checkpoint_recorded, %{checkpoint_id: checkpoint.id, epoch: checkpoint.epoch, filesystem_digest: checkpoint.filesystem_digest, trust_level: checkpoint.trust_level})
  ...
  def handle_call({:latest_stable_checkpoint, episode_id}, _from, state) do
    checkpoint = state.checkpoints |> Map.values() |> Enum.filter(&(&1.episode_id == episode_id and &1.trust_level == :stable)) |> Enum.max_by(& &1.created_at, fn -> nil end)
  ...
  def handle_call({:update_trajectory, episode_id, homeostat}, _from, state) do
    with {:ok, episode} <- fetch_map(state.episodes, episode_id, :episode_not_found),
      ...
      episode = %{episode | trajectory_version: homeostat.trajectory_version, trajectory_regime: homeostat.regime, trajectory_state: Map.from_struct(homeostat)}
      state = put_in(state, [:episodes, episode_id], episode)
      {_seq, state} = add_event(state, episode_id, :trajectory_updated, %{version: homeostat.trajectory_version, regime: homeostat.regime})
      {:reply, :ok, state}
    else
  ...
  def handle_call({:claim_episode, id, owner, ttl}, _from, state) do
    with {:ok, episode} <- fetch_map(state.episodes, id, :episode_not_found) do
      now = Canonical.now()
      if is_nil(episode.owner_lease_expires_at) or episode.owner_lease_expires_at < now or episode.owner_node == owner do
        token = Canonical.id()
        episode = %{episode | owner_node: owner, owner_lease_token: token, owner_lease_expires_at: now + ttl}
  ...
  def handle_call({:append_event, id, kind, payload}, _from, state) do
    case state.episodes[id] do
      nil -> {:reply, {:error, :episode_not_found}, state}
      _ ->
  ...
  defp fresh do
    %{episodes: %{}, leases: %{}, effects: %{}, decisions: %{}, checkpoints: %{}, events: %{}, observations: [], recoveries: %{}}
  end

  defp fetch_map(map, key, reason), do: case Map.fetch(map, key) do {:ok, value} -> {:ok, value}; :error -> {:error, reason} end
  ...
  defp validate_decision(episode, effect, decision) do
    ...
    checks = [
      ...
      {Map.get(decision, :policy_version, effect.version_vector.policy_version) == effect.version_vector.policy_version, :stale_decision_policy},
      {Map.get(decision, :trajectory_version, effect.version_vector.trajectory_version) == effect.version_vector.trajectory_version, :stale_decision_trajectory},
    ...
    case Enum.find(checks, fn {ok, _} -> not ok end) do nil -> :ok; {_, reason} -> {:error, reason} end
  end

  defp same_decision?(a, b) do
    a.decision == b.decision and a.epoch == b.epoch and a.policy_version == b.policy_version and a.trajectory_version == b.trajectory_version and a.payload_digest == b.payload_digest and a.evidence_ref == b.evidence_ref
  end
  ...
  defp validate_required_decisions(decisions, effect, required) do
    ...
    if Enum.any?(current, &(&1.decision == "deny")) do
      ...
      valid = current |> Enum.filter(fn d -> d.decision == "allow" and d.epoch == effect.epoch and d.policy_version == effect.version_vector.policy_version and d.trajectory_version == effect.version_vector.trajectory_version and d.payload_digest == effect.payload_digest and (is_nil(d.expires_at) or d.expires_at > now) end) |> MapSet.new(& &1.kind)
  ...
  defp put_receipt(metadata, receipt), do: Map.put(metadata || %{}, "external_receipt", plain(receipt))

  defp add_event(state, episode_id, kind, payload) do
    prior = Map.get(state.events, episode_id, [])
    seq = length(prior) + 1
    previous_hash = case List.last(prior) do nil -> nil; event -> event.entry_hash end
    body = %{episode_id: episode_id, sequence: seq, kind: to_string(kind), payload: plain(payload), previous_hash: previous_hash, created_at_ms: Canonical.now()}

...
first =
  Support.prepare!(spec, runtime, :http_read, "same bytes")
  |> Support.evaluate!()
  |> Support.commit!()
...
Support.assert!(
  first.id != second.id,
  "current broker models repeated proposals as distinct logical effects"
)

Support.assert_equal!(
  first.payload_digest,
  second.payload_digest,
  "content identity is digest-stable"
)

Support.assert_equal!(
  second_receipt["mode"] || second_receipt[:mode],
  "idempotent_replay",
  "target conditional write must identify replay"
)
...
Support.assert!(
  match?({:ok, %{state: :commit_unknown}}, result),
  "post-intent adapter conflict is conservatively commit_unknown"
)

IO.puts(
  "ASSERTION PASSED: digest identity is stable; target replay is idempotent; ambiguous conflicts do not clobber"
)

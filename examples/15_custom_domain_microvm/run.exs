...
defmodule ExampleMicroVMBackend do
  ...
  def exec(%ExecutionDomain.Domain{}, %ExecutionDomain.Command{}), do: {:error, :backend_not_configured}
...
spec = %ExecutionDomain.Spec{episode_id: Canonical.id(), epoch: 1, workspace: %{}, hard_envelope: %{}, resource_limits: %{}, environment: %{}, effect_socket: nil}
:ok = ExecutionDomainCase.verify_skeleton!(ExampleMicroVMBackend, spec)
Support.assert_equal!(ExampleMicroVMBackend.create(spec), {:error, :backend_not_configured}, "unconfigured backend must not pretend isolation")
IO.puts("Backend author owns: stale-generation rejection, launch isolation, checkpoint generation fencing, cgroup/VM-wide destruction, and proof of emptiness before cleanup.")
IO.puts("ASSERTION PASSED: the skeleton fails closed until a real microVM/systemd-nspawn backend proves the required invariants")

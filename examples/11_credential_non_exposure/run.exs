...
{:ok, effect} = EffectBroker.prepare(%{
  episode_id: spec.id,
...
Support.assert!(not Map.has_key?(effect.target, "api_key") and not Map.has_key?(effect.target, "token"), "credential fields must be stripped from the public target")
Support.assert!(not String.contains?(serialized, secret), "secret must not appear in persisted episode/effect/event artifacts")
Support.assert_equal!(target["credential_env"], "AUTONOMIC_EXAMPLE_TOKEN", "trusted side stores only credential environment name")
System.delete_env("AUTONOMIC_EXAMPLE_TOKEN")
IO.puts("ASSERTION PASSED: the broker-owned credential value never enters worker-visible or persisted effect data")

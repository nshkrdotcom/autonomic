...
{spec, runtime} = Support.start_episode!(max_class: :class_4_irreversible_high_impact, kinds: ["publish"])

make_effect = fn payload -> Support.prepare!(spec, runtime, :publish, payload, reversible?: false) end
...
wrong_doc = %{wrong_doc | signature: Autonomic.Policy.sign(wrong_doc.document, "operator", private, "human-approval")["signature"]}
...
Application.put_env(:autonomic, :decision_keys, %{"operator" => Base.encode64(public), "not-used" => Base.encode64(other_public)})
...
Support.assert_equal!(missing_result, {:error, :human_approval_required}, "missing approval denial")
Support.assert_equal!(mismatch_result, {:error, :invalid_human_approval}, "mismatched digest denial")
...
IO.puts("ASSERTION PASSED: Class 4 approval is bound to exact effect/revision/epoch/payload and trusted key")

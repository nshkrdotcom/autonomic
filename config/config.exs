...
config :autonomic_kernel,
  ...
  targets: %{}, policy_keys: %{}, decision_keys: %{},
  effect_concurrency: 8, sensor_queue: 64, speculation_ms: 15_000,
  max_repairs: 3, owner_ttl_ms: 15_000, automatic_verification: true
...
config :autonomic_store, Autonomic.Store.Repo,
  ...
  pool_size: 12, queue_target: 1000, queue_interval: 1000,
  migration_primary_key: [name: :id, type: :string]
config :autonomic_linux, enabled: false,
  executable: Path.expand("../native/autonomic_launcher/target/release/autonomic_launcher", __DIR__), sudo: true,
  rootfs: "/var/lib/autonomic/rootfs", state_root: "/var/lib/autonomic"
config :autonomic_typesafe, timeout_ms: 3000, required_capabilities: [],
  allowed_models: [], model: "jev-latest", evidence_limit: 32_768,

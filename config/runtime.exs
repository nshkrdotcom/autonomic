import Config
if url = System.get_env("DATABASE_URL"), do: config(:autonomic_store, Autonomic.Store.Repo, url: url)
if root = System.get_env("AUTONOMIC_STATE_DIR"), do: config(:autonomic_kernel, state_dir: Path.expand(root))
...
if path = System.get_env("AUTONOMIC_ROOTFS"), do: config(:autonomic_linux, rootfs: Path.expand(path))
if path = System.get_env("AUTONOMIC_LINUX_STATE_ROOT"), do: config(:autonomic_linux, state_root: Path.expand(path))
...
if file = System.get_env("AUTONOMIC_CONFIG") do
  ...
  config :autonomic_kernel, targets: Map.fetch!(document, "targets"),
    policy_keys: Map.fetch!(document, "policy_keys"),
    decision_keys: Map.fetch!(document, "decision_keys"),
    signer: Map.fetch!(document, "signer"), domain_spec: Map.fetch!(document, "domain_spec")
  config :autonomic_typesafe,
    allowed_models: Map.get(document, "allowed_models", []),
    required_capabilities: Enum.map(Map.get(document, "required_capabilities", []), fn

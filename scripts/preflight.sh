...
rootfs=${AUTONOMIC_ROOTFS:-/opt/autonomic/rootfs}
...
launcher=${AUTONOMIC_LAUNCHER:-/usr/local/libexec/autonomic_launcher}
...
{ [[ -d "$state_root" && -w "$state_root" ]] || sudo -n test -w "$state_root"; } && echo "ok   state root                $state_root" || { echo "FAIL state root                $state_root not writable"; failed=1; }

if [[ -n ${AUTONOMIC_TEST_DATABASE_URL:-} ]]; then
  psql "${AUTONOMIC_TEST_DATABASE_URL/ecto:\/\//postgresql:\/\/}" -Atqc 'select 1' >/dev/null && echo 'ok   PostgreSQL test DB         reachable' || { echo 'FAIL PostgreSQL test DB         unreachable'; failed=1; }
else

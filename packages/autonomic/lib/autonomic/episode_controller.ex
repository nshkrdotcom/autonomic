defmodule Autonomic.EpisodeController do
  ...
  @doc false
  def trusted_runtime(episode_id),
    do: :gen_statem.call(Runtime.via(episode_id, :controller), :trusted_runtime)
  ...
  def contain(episode_id, reason),
    do: :gen_statem.cast(Runtime.via(episode_id, :controller), {:contain, reason})
  ...
  def bootstrapping(:internal, :bootstrap, data) do
    ...
    with {:ok, socket} <- effect_socket(data.episode_id),
         ...
         {:ok, checkpoint} <-
           SnapshotManager.capture(data.episode_id, domain,
             reason: :bootstrap,
             trust_level: :stable,
             git_base_ref: workspace_base(data.spec.workspace)
           ),
  ...
  def bootstrapping(:internal, {:recover_controller, old_domain}, data) do
    _ =
      Runtime.store().set_episode_state(data.episode_id, :repairing, %{
        reason: :controller_restart
      })

    with {:ok, result} <-
           RepairManager.repair(data.episode_id, old_domain, data.spec, :controller_restart, %{
             deterministic: [%{type: :controller_restart}]
           }),
         :ok <- Runtime.store().set_episode_domain(data.episode_id, result.domain),
         :ok <-
           Runtime.store().set_episode_state(data.episode_id, :running, %{
             recovery_id: result.recovery_id
           }) do
      next = %{
        data
        | epoch: result.domain.epoch,
          domain: result.domain,
          lease: result.lease,
          checkpoint: result.checkpoint,
          repairs: data.repairs + 1,
          last_signal: nil
      }
  ...
  def bootstrapping({:call, from}, :state, data),
    do: {:keep_state_and_data, [{:reply, from, {:bootstrapping, public(data)}}]}
  ...
  def running(:internal, :launch_worker, data) do
    case start_worker(data) do
      {:ok, ref} ->
        {:keep_state, %{data | worker_ref: ref}}

      :disabled ->
        :keep_state_and_data

      {:error, reason} ->
        {:next_state, :preempting, %{data | last_signal: {:worker_start_failed, reason}},
         [
           {:next_event, :internal,
            {:repair, :environment_mismatch, %{worker_start_failed: inspect(reason)}}}
         ]}
    end
  end

  def running({:call, from}, :state, data),
    do: {:keep_state_and_data, [{:reply, from, {:running, public(data)}}]}

  def running({:call, from}, :trusted_runtime, data),
    do:
      {:keep_state_and_data,
       [
         {:reply, from,
  ...
  def running(:cast, {:homeostat, :yield, reason, evidence}, data) do
    _ =
      Runtime.store().set_episode_state(data.episode_id, :yielding, %{
        reason: reason,
        evidence: evidence
      })
  ...
  def running(:cast, {:homeostat, :preempt, {:semantic_exit, reason, evidence}}, data) do
    ...
    {:next_state, :preempting, %{data | last_signal: {reason, evidence}},
     [{:next_event, :internal, {:repair, reason, evidence}}]}
  end
  ...
  def running(:cast, {:contain, reason}, data) do
    {:next_state, :containing, %{data | last_signal: {:operator_contain, reason}},
     [{:next_event, :internal, {:contain, reason}}]}
  end
  ...
  def running(:info, {:worker_exit, status}, data) do
    if privileged_in_flight?(data.episode_id) do
      {:next_state, :preempting, data,
       [{:next_event, :internal, {:repair, :environment_mismatch, %{worker_exit: status}}}]}
    else
  ...
  def yielding({:call, from}, :state, data),
    do: {:keep_state_and_data, [{:reply, from, {:yielding, public(data)}}]}
  ...
  def yielding(:cast, {:contain, reason}, data),
    do: {:next_state, :containing, data, [{:next_event, :internal, {:contain, reason}}]}

  def yielding(_type, _event, _data), do: :keep_state_and_data

  def preempting(:internal, {:repair, reason, evidence}, data) do
    max_repairs = Application.get_env(:autonomic, :max_repairs, 3)

    if data.repairs >= max_repairs do
      {:next_state, :containing, data,
       [{:next_event, :internal, {:contain, :repair_budget_exhausted}}]}
    else
  ...
  def preempting({:call, from}, :state, data),
    do: {:keep_state_and_data, [{:reply, from, {:preempting, public(data)}}]}

  def preempting(_type, _event, _data), do: :keep_state_and_data

  def repairing(:internal, {:repair, reason, evidence}, data) do
    with {:ok, result} <-
           RepairManager.repair(data.episode_id, data.domain, data.spec, reason, evidence),
         :ok <- Runtime.store().set_episode_domain(data.episode_id, result.domain),
         :ok <-
           Runtime.store().set_episode_state(data.episode_id, :running, %{
             recovery_id: result.recovery_id
      ...
      {:error, reason} ->
        {:next_state, :containing, data,
         [{:next_event, :internal, {:contain, {:repair_failed, reason}}}]}
    end
  end

  def repairing({:call, from}, :state, data),
    do: {:keep_state_and_data, [{:reply, from, {:repairing, public(data)}}]}
  ...
  def containing(:internal, {:contain, reason}, data) do
    ...
    case destroy do
      :ok ->
        ...
        _ =
          Runtime.store().set_episode_state(data.episode_id, :contained, %{
            reason: inspect(reason),
            destruction_proven: true
          })
      ...
      {:error, destroy_reason} ->
        fail(data, {:containment_failed, destroy_reason})
    end
  end

  def containing({:call, from}, :state, data),
    do: {:keep_state_and_data, [{:reply, from, {:containing, public(data)}}]}

  def containing(_type, _event, _data), do: :keep_state_and_data

  def contained({:call, from}, :state, data),
    do: {:keep_state_and_data, [{:reply, from, {:contained, public(data)}}]}
  ...
  def completing(:internal, :finalize, data) do
    with false <- privileged_in_flight?(data.episode_id),
         ...
         :ok <-
           Runtime.store().set_episode_state(data.episode_id, :completed, %{
             completed_at: Canonical.now()
           }) do
  ...
  def completing({:call, from}, :state, data),
    do: {:keep_state_and_data, [{:reply, from, {:completing, public(data)}}]}

  def completing(_type, _event, _data), do: :keep_state_and_data

  def completed({:call, from}, :state, data),
    do: {:keep_state_and_data, [{:reply, from, {:completed, public(data)}}]}

  def completed(_type, _event, _data), do: :keep_state_and_data

  def failed({:call, from}, :state, data),
    do: {:keep_state_and_data, [{:reply, from, {:failed, public(data)}}]}
  ...
  defp issue_initial_lease(data) do
    capabilities =
      Map.get(
        data.spec.hard_envelope,
        "capabilities",
        Map.get(data.spec.hard_envelope, :capabilities, [])
      )

    ceiling =
      data.spec.requested_effect_ceiling ||
        Map.get(data.spec.hard_envelope, "max_effect_class", :class_1_isolated_mutable)
  ...
  defp ensure_epoch_fence(data, reason) do
    case Runtime.store().current_epoch(data.episode_id) do
      {:ok, epoch} when epoch == data.epoch ->
        AuthorityGovernor.advance_epoch(data.episode_id, {:contain, reason})
  ...
  defp resource_limit(limits, key, default),
    do: Map.get(limits, key, Map.get(limits, to_string(key), default))
  ...
  defp start_worker(%{spec: %{worker_argv: argv}}) when not is_list(argv) or argv == [],
    do: :disabled
  ...
  defp worker_status({:ok, execution}), do: execution.exit_status
  defp worker_status({:error, reason}), do: {:error, reason}
  ...
  defp public(data),
    do:
      Map.take(data, [:episode_id, :epoch, :lease, :checkpoint, :repairs, :last_signal])
      |> Map.put(
        :domain,
        if(data.domain,
          do: %{id: data.domain.id, epoch: data.domain.epoch, generation: data.domain.generation},
          else: nil
        )
      )
end

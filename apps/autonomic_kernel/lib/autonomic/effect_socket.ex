defmodule Autonomic.EffectSocket do
  ...
  defp decode_request(payload) do
    with {:ok, request} <- Jason.decode(payload),
         ...
         action when action in ["prepare", "evaluate", "commit", "abort", "status", "fetch_response"] <- Map.get(request, "action") do
      {:ok, request}
    else
  ...
  defp dispatch_current(request, episode_id) do
    case request["action"] do
      ...
      "status" -> effect_for_episode(request["effect_id"], episode_id)
      ...
      "fetch_response" ->
        with {:ok, effect} <- effect_for_episode(request["effect_id"], episode_id),
             ...
             ref when is_binary(ref) <- Map.get(receipt, "response_ref") || Map.get(receipt, :response_ref),
          ...
          {:ok, %{offset: offset, bytes: byte_size(chunk), total_bytes: byte_size(bytes), body_digest: Autonomic.Canonical.hash(bytes), eof: offset + byte_size(chunk) >= byte_size(bytes), chunk_b64: Base.encode64(chunk)}}
        else
  ...
  defp bounded_integer(value, default, low, high) when is_integer(value), do: value |> max(low) |> min(high)
  ...
  defp encode_response({:error, reason}), do: Jason.encode!(%{ok: false, error: inspect(reason, limit: 20, printable_limit: 1_024)})
  defp encode_response(other), do: Jason.encode!(%{ok: false, error: inspect(other, limit: 20, printable_limit: 1_024)})
  ...
  defp safe_result(%Autonomic.ProposedEffect{} = effect) do
    ...
    %{id: effect.id, episode_id: effect.episode_id, epoch: effect.epoch, revision: effect.revision, class: effect.class, kind: effect.kind, state: effect.state, payload_digest: effect.payload_digest, commit_attempt_id: effect.commit_attempt_id, committed_at: effect.committed_at, external_receipt_ref: effect.external_receipt_ref, receipt: receipt}
  end
  defp safe_result(value) when is_map(value), do: value |> Map.drop([:failure, "failure", :metadata, "metadata"])
  ...
  defp recv_exact(socket, remaining, acc) do
    case :socket.recv(socket, remaining, 15_000) do
      {:ok, data} when byte_size(data) > 0 -> recv_exact(socket, remaining - byte_size(data), acc <> data)

defmodule Autonomic.VerificationRunner do
  ...
  defp run_snapshot(effect, target, snapshot) do
    ...
    spec = %ExecutionDomain.Spec{
      ...
      resource_limits:
        Map.get(target, "verification_resource_limits", %{
          cpu_quota: 2,
          memory_mb: 2048,
          pids: 192
        }),
      ...
      metadata: %{
        purpose: :trusted_slow_verification,
        effect_id: effect.id,
    ...
    with {:ok, domain} <- domain_backend.create(spec) do
      try do
        commands =
          [
            ["git", "apply", "--check", "--whitespace=error", ".autonomic-effect.patch"],
            ["git", "apply", "--whitespace=error", ".autonomic-effect.patch"]
          ] ++ verification_commands(target)
  ...
  defp run_commands(domain, [argv | rest], env, evidence) do
    ...
    case domain.backend.exec(domain, command) do
      {:ok, execution} when execution.exit_status == 0 ->
        item = command_evidence(argv, execution)

        run_commands(domain, rest, env, [item | evidence])

      {:ok, execution} ->
        {:error,
         {:verification_command_failed, Canonical.digest(argv), execution.exit_status,
          Canonical.hash(execution.metadata[:stdout] || ""),
          Canonical.hash(execution.metadata[:stderr] || "")}}
  ...
  defp command_evidence(argv, execution) do
    %{
  ...
  defp export_snapshot(repo, base, effect) do
    root =
      Path.join([
        ...
        "verification",
        effect.episode_id,
        effect.id <> "-" <> Integer.to_string(effect.revision)
      ])
    ...
    try do
      ...
      result =
        with {_, 0} <-
               System.cmd("git", ["archive", "--format=tar", "--output=#{archive}", base],
                 cd: repo,
             ...
             {_, 0} <-
               System.cmd(
                 "tar",
                 ["--no-same-owner", "--no-same-permissions", "-xf", archive, "-C", snapshot],
                 stderr_to_stdout: true
               ),
             {:ok, patch} <- Autonomic.Payloads.get(effect.episode_id, effect.payload_ref),
             :ok <-
               File.write(Path.join(snapshot, ".autonomic-effect.patch"), patch, [
                 :binary,
                 :exclusive
               ]) do
          {:ok, snapshot, cleanup}
          ...
          {output, status} when is_binary(output) and is_integer(status) ->
            {:error, {:snapshot_export_failed, status, Canonical.hash(output)}}
  ...
  defp verification_commands(target) do
    case Map.get(target, "verify_argv") do
      commands when is_list(commands) and commands != [] ->
        if Enum.all?(commands, &is_binary/1),
          do: [validate_argv!(commands)],
          else: Enum.map(commands, &validate_argv!/1)
  ...
  defp validate_argv!(argv) when is_list(argv) and argv != [] do
    if length(argv) <= 256 and
         Enum.all?(argv, &(is_binary(&1) and &1 != "" and byte_size(&1) <= 16_384)),
       do: argv,
       else: raise(ArgumentError, "invalid verify_argv")
  end

  defp validate_argv!(_), do: raise(ArgumentError, "invalid verify_argv")

  defp verification_env_entry({key, value}) do
    key = to_string(key)
    value = to_string(value)
    lowered = String.downcase(key)

    if byte_size(key) + byte_size(value) > 32_768,
      do: raise(ArgumentError, "verification environment entry too large")

    if Enum.any?(
         ["token", "secret", "password", "credential", "api_key", "authorization"],
         &String.contains?(lowered, &1)
       ),
       do: raise(ArgumentError, "verification environment may not contain credentials")
  ...
  defp verification_env(target) do
    case Map.get(target, "verification_env", %{}) do
      env when is_map(env) ->
        Map.new(env, &verification_env_entry/1)

      _ ->
        raise ArgumentError, "verification_env must be a map"
    end
  end
    ...
    case domain.backend.destroy(domain) do
      ...
      {:ok, evidence} ->
        if(Map.get(evidence, :empty, Map.get(evidence, "empty", false)),
          do: :ok,
          else: {:error, :verification_domain_not_empty}
        )
  ...
  defp repo_path(target) do
    case Map.get(target, "repo") do
      ...
      _ ->
        {:error, :trusted_repo_not_configured}
    end
  end
  ...
  defp assert_base(repo, base) do
    case System.cmd("git", ["rev-parse", "--verify", "#{base}^{commit}"],
           cd: repo,
           stderr_to_stdout: true
         ) do
  ...
  defp verification_timeout,
    do: Application.get_env(:autonomic_kernel, :verification_timeout_ms, 120_000)
end

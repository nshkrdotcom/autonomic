defmodule Autonomic.Store.Repo.Migrations.CreateAutonomicTables do
  ...
  def change do
    ...
    create(
      constraint(:episodes, :trajectory_version_nonnegative, check: "trajectory_version >= 0")
    )
    ...
    create(
      index(:capability_leases, [:episode_id, :epoch, :revoked_at_ms, :expires_at_ms],
        name: :leases_authority_lookup
      )
    )
    ...
    create(
      unique_index(:effect_decisions, [:effect_id, :effect_revision, :kind, :source_ref],
        name: :effect_decision_identity
      )
    )

    create(index(:effect_decisions, [:effect_id, :effect_revision, :decision]))

    create table(:episode_events, primary_key: false) do
    ...
    create table(:observation_frames, primary_key: false) do
      add(:id, :bigserial, primary_key: true)
      add(:episode_id, references(:episodes, type: :string, on_delete: :delete_all), null: false)
      add(:epoch, :bigint, null: false)
      add(:sequence, :bigint, null: false)

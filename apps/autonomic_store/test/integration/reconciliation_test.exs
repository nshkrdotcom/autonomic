defmodule Autonomic.Store.ReconciliationTest do
  ...
  setup do
    ...
    old_state = Application.fetch_env!(:autonomic_kernel, :state_dir)
    Application.put_env(:autonomic_kernel, :targets, %{"repo" => target})
    Application.put_env(:autonomic_kernel, :state_dir, state)

    on_exit(fn ->
      Application.put_env(:autonomic_kernel, :targets, old_targets)
      Application.put_env(:autonomic_kernel, :state_dir, old_state)
      File.rm_rf(root)
    end)

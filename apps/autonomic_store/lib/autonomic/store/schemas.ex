defmodule Autonomic.Store.Schema.Episode do
  ...
  def changeset(record, attrs) do
    record
    |> cast(attrs, [
      :id, :state, :current_epoch, :policy_id, :policy_version, :policy_digest, :policy,
      :hard_envelope, :origin_intent_digest, :current_checkpoint_id, :domain_ref, :domain_generation,
      :domain_os_ref, :domain_metadata, :trajectory_version, :trajectory_regime, :trajectory_state, :owner_node, :owner_lease_token,
      :owner_lease_expires_at, :metadata
    ])
    |> validate_required([:id, :state, :current_epoch, :policy_version, :policy_digest, :policy, :hard_envelope, :trajectory_version, :trajectory_regime])
...
defmodule Autonomic.Store.Schema.CapabilityLease do
  ...
  def changeset(record, attrs) do
    record
    |> cast(attrs, [:id, :episode_id, :epoch, :policy_version, :capabilities, :max_effect_class,
      :issued_at_ms, :expires_at_ms, :revoked_at_ms, :authority_source, :authority_ref, :reason, :metadata])
    |> validate_required([:id, :episode_id, :epoch, :policy_version, :capabilities, :max_effect_class,
      :issued_at_ms, :expires_at_ms, :authority_source])
  end
end
...
defmodule Autonomic.Store.Schema.Checkpoint do
  ...
  def changeset(record, attrs) do
    record
    |> cast(attrs, [:id, :episode_id, :epoch, :domain_generation, :parent_checkpoint_id,
      :filesystem_ref, :filesystem_digest, :git_base_ref, :git_patch_digest,
      :trajectory_version, :trajectory_ref, :policy_version, :capability_state_ref,
      :environment_digest, :dependency_lock_digest, :trust_level, :created_at_ms,
      :pending_effect_ids, :metadata])
    |> validate_required([:id, :episode_id, :epoch, :domain_generation, :filesystem_ref,
      :filesystem_digest, :trajectory_version, :policy_version, :created_at_ms])
  end
end
...
defmodule Autonomic.Store.Schema.Effect do
  ...
  def changeset(record, attrs) do
    record
    |> cast(attrs, [:id, :episode_id, :epoch, :lease_id, :revision, :class, :kind, :target,
      :payload_ref, :payload_digest, :reversible, :state, :version_vector, :idempotency_key,
      :commit_attempt_id, :version_vector_at_commit, :commit_started_at_ms, :expires_at_ms,
      :committed_at_ms, :external_receipt_ref, :external_receipt, :failure, :created_at_ms, :metadata])
    |> validate_required([:id, :episode_id, :epoch, :lease_id, :revision, :class, :kind, :target,
      :payload_ref, :payload_digest, :reversible, :state, :version_vector, :created_at_ms])
...
defmodule Autonomic.Store.Schema.EffectDecision do
  ...
  def changeset(record, attrs) do
    record
    |> cast(attrs, [:id, :effect_id, :effect_revision, :kind, :decision, :source_ref,
      :episode_id, :epoch, :policy_version, :trajectory_version, :snapshot_ref,
      :expires_at_ms, :payload_digest, :evidence_ref, :signature, :created_at_ms, :metadata])
    |> validate_required([:id, :effect_id, :effect_revision, :kind, :decision, :source_ref, :episode_id,
      :epoch, :policy_version, :trajectory_version, :payload_digest, :created_at_ms])
...
defmodule Autonomic.Store.Schema.EpisodeEvent do
  ...
  def changeset(record, attrs) do
    record
    |> cast(attrs, [:episode_id, :sequence, :kind, :payload, :previous_hash, :entry_hash, :created_at_ms])
...
defmodule Autonomic.Store.Schema.ObservationFrame do
  ...
  def changeset(record, attrs) do
    record
    |> cast(attrs, [:episode_id, :epoch, :sequence, :observed_at_ms, :deterministic, :semantic,
      :resource, :effect_context, :dropped_low_priority_count, :evidence_digest, :metadata])
...
defmodule Autonomic.Store.Schema.RecoveryRecord do
  ...
  def changeset(record, attrs) do
    record
    |> cast(attrs, [:id, :episode_id, :from_epoch, :to_epoch, :checkpoint_id, :old_domain_ref,
      :new_domain_ref, :reason, :evidence_ref, :created_at_ms, :metadata])
